"""
Unit tests for `pabutools.recommendation`, the implementation of the algorithms in
...
def requires(*modules):
    ...
    return skipUnless(
        not missing,
        f"needs {', '.join(missing)} (pip install pabutools[recommendation])",
    )
...
def require_predictor(predictor):
    ...
    if not _installed(module):
        raise SkipTest(
            f"needs {module} (pip install pabutools[recommendation])"
        )
