...
optional ``recommendation`` libraries, which the library's own CI does not
install; those are skipped when their library is absent, exactly as the
equivalent unit tests are.
"""
...
from pabutools.analysis import recommendationanalytics
from pabutools.recommendation import model_training, recommendation

#: The modules whose doctests are collected.
MODULES = (recommendation, model_training, recommendationanalytics)
...
class TestDoctests(TestCase):
    ...
    def test_doctest(self, name, test):
        ...
        if library is not None and not _installed(library):
            self.skipTest(
                f"needs {library} (pip install pabutools[recommendation])"
            )
        failures = []
