...
:py:mod:`~pabutools.recommendation.recommendation` runs the system - partial
...
treatment matrix that reports them.
    ...
    * FN = hidden projects she really approves but we predicted reject,
    ...
    ----------
        ...
        hidden : set[:py:class:`~pabutools.election.instance.Project`]
            ...
            :py:func:`~pabutools.recommendation.recommendation.hidden_projects`).
...
def fractional_allocation_score(
    ...
    ----------
        real_bundle : Iterable[:py:class:`~pabutools.election.instance.Project`]
            ...
            returned by :py:func:`~pabutools.recommendation.recommendation.greedy_approval`,
            as is.
...
def run_all_experiments(
    ...
    *,
    ...
    :py:func:`~pabutools.recommendation.recommendation.split_lv_tv` (which also
    derives k, the number of questions per TV voter, from the two degrees -
    Section 3.0.1), the pipeline is run with
    :py:func:`~pabutools.recommendation.recommendation.run_pipeline`, and the
    ...
    ----------
        ...
        setups : Iterable[str], optional
            ...
            :py:data:`~pabutools.recommendation.recommendation.SETUPS`).
        predictors : Iterable[str], optional
            The predictors to sweep (names from
            :py:data:`~pabutools.recommendation.recommendation.PREDICTORS`).
